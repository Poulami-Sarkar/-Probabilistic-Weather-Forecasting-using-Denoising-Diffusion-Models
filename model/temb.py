...
class TimeEmbeddedUNet2DConditionModel(UNet2DConditionModel):
    def __init__(self,
        ...
        self.day_proj = DateTime_Transform(num_channels = block_out_channels[0]/2,flip_sin_to_cos=flip_sin_to_cos,downscale_freq_shift=1)        
        self.hour_proj = DateTime_Transform(num_channels = block_out_channels[0]/2,flip_sin_to_cos=flip_sin_to_cos,downscale_freq_shift=1)

        self.datetime_embedding = TimestepEmbedding(
    ...
    def forward(
        ...
        datetime_emd = torch.cat([d_emb,h_emb], dim=-1)
        datetime_emd = datetime_emd.to(dtype=sample.dtype)
        dt_emb = self.datetime_embedding(datetime_emd)
